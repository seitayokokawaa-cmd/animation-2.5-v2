...
export function checkReferences(doc: MfsDocument, loaded: LoadedYaml, file: string): Finding[] {
  ...
  const shapeNames = Object.keys(doc.shapes);
    ...
    scene.place.forEach((place, pi) => {
      if (!shapeNames.includes(place.ref)) {
        findings.push({
          ...
          message: `Scene "${scene.id}" places unknown shape "${place.ref}"`,
          hint: `Define it under shapes: or fix the name.${suggest(place.ref, shapeNames)}`,
        });
      }
...
/** T1 + T2 in one call — the `mf check` entry point for v0. */
export function check(text: string, file: string): CheckResult {
  const t1 = checkStructure(text, file);
  if (!t1.doc) return t1;
  const findings = [...t1.findings, ...checkReferences(t1.doc, t1.loaded, file)];
  return { ...t1, findings };
}
